...
function makeSimulation_walk(simulationHomeDir)
    ...
    system('scale -S scale_setup_walk_scaleOnly.xml');
    system('scale -S scale_setup_walk.xml');
    
    % IK
    cd([simulationHomeDir,'/IK'])
    system('ik -S ik_setup_walk.xml');
    
    % RRA
    cd([simulationHomeDir,'/RRA/walk']);
    
    system('rra -S rra_setup_walk_1.xml');
    ...
    system('rra -S rra_setup_walk_2.xml');
    outlog = fileread('out.log');
    massChange = str2double(outlog(regexpi(outlog, 'total mass change: ', 'end'):regexpi(outlog, 'total mass change: .?[0-9]+[.][0-9]+', 'end')));
    ...
    system('cmc -S cmc_setup_walk.xml');
    
    % ID
    cd([simulationHomeDir,'/ID']);
    system('id -S id_setup_walk.xml');
...
function makeSimulation_run(simulationHomeDir)
    ...
    % Scale
    cd([simulationHomeDir,'/Scale'])
    system('scale -S scale_setup_run.xml');
    
    % IK
    cd([simulationHomeDir,'/IK'])
    system('ik -S ik_setup_run.xml');
    
    % RRA
    cd([simulationHomeDir,'/RRA/run']);
    
    system('rra -S rra_setup_run_1.xml');
    outlog = fileread('out.log');
    massChange = str2double(outlog(regexpi(outlog, 'total mass change: ', 'end'):regexpi(outlog, 'total mass change: .?[0-9]+[.][0-9]+', 'end')));
    ...
    system('rra -S rra_setup_run_2.xml');
    outlog = fileread('out.log');
    massChange = str2double(outlog(regexpi(outlog, 'total mass change: ', 'end'):regexpi(outlog, 'total mass change: .?[0-9]+[.][0-9]+', 'end')));
    ...
    system('rra -S rra_setup_run_3.xml');
    outlog = fileread('out.log');
    massChange = str2double(outlog(regexpi(outlog, 'total mass change: ', 'end'):regexpi(outlog, 'total mass change: .?[0-9]+[.][0-9]+', 'end')));
    ...
    system('cmc -S cmc_setup_run.xml');
    
    % ID
    cd([simulationHomeDir,'/ID']);
    system('id -S id_setup_run.xml');
end
