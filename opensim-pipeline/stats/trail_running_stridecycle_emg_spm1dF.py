    ...
    for d in data_type:
        ...
        for a in analyses:
            ...
            for v in osimvars[a]:
                ...
                # Group 3: ctrl
                gp2data = df[~df["subject"].isin(exclusions) & (df["is_surgical"] == issurgical[2]) & (df["analysis"] == a) & (df["variable"] == v)]
                datamat[n][d][a][v][2] = gp2data.loc[:, "t1":"t101"].to_numpy()
    ...
    for d in data_type:
        ...
        for a in analyses: 
            ...
            for v in osimvars[a]:
                ...
                Y2 = datamat[n][d][a][v][2]
                spmt[n][d][a][v] = spm1d.stats.anova1((Y0, Y1, Y2), equal_var=True)
                spmtinf[n][d][a][v] = spmt[n][d][a][v].inference(alpha = significance[bonferroni], interp=True)
...
plotsubjtype = ["symptomatic", "asymptomatic", "control"]
...
for n in condition:
    ...
    for nd, d in enumerate(data_type):    
        ...
        for a in osimvars:
            ...
            for col, v in enumerate(osimvars[a]):                
                ...
                ax.fill_between(x, l2, u2, alpha = 0.3, linewidth = 0.0, color = "green")
                ax.fill_between(x, l1, u1, alpha = 0.3, linewidth = 0.0, color = "blue")
                ax.fill_between(x, l0, u0, alpha = 0.3, linewidth = 0.0, color = "red")
                ax.plot(x, m2, label = plotsubjtype[2], linewidth = 2.0, color = "green") 
                ax.plot(x, m1, label = plotsubjtype[1], linewidth = 2.0, color = "blue") 
    ...
    # save to pdf
    plt.savefig(os.path.join(outpath, outfilename + "_" + n + "_spm1dF.pdf"))